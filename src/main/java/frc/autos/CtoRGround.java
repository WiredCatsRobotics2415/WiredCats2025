...
public class CtoRGround extends GenericAuto {
    ...
    private Command auto = new AlignToReef(Side.Left).alongWith(new ReefPresetTo(Level.L4)).withTimeout(4)
        .andThen(new InstantCommand(() ->
        {
            ...
            followPath = paths.stream().filter(path -> path.name.equals("LeftIntakeFinal")).findFirst().orElse(null);
            System.out.println(followPath.name);
            AutoBuilder.followPath(followPath).schedule();
        })).andThen(new WaitUntilCommand(() -> atTarget)).andThen(new InstantCommand(() -> {
            ...
            .andThen(new InstantCommand(() ->
            {
                ...
                followPath = paths.stream().filter(path -> path.name.equals("L4FarBackup")).findFirst().orElse(null);
                System.out.println(followPath.name);
                AutoBuilder.followPath(followPath).schedule();
                superstructure.stow();
