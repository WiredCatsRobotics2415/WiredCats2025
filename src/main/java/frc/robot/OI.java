...
public static class OI {
  ...
  public void setPreferences() {
    if (RobotPreferences.getInputFilter()) {
      ...
      if (curve < 1)
        curve = 1; // Note: must include because fractional/negative powers will yield uncontrolable
      // results
    } else {
      // Slew
      isCurve = false;
      ...
      if (slewRate < 0)
        slewRate =
            1.0d; // Note: must include because negative rates will yield uncontrolable results
